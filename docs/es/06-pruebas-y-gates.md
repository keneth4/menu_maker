...
## Estado actual (sync docs 2026-02-20)
- `npm run build`: PASS
- `npm test`: PASS (`67` files, `230` tests)
- `npm run test:e2e`: no re-ejecutado en este sync documental (el ultimo sweep historico seguia fallando; ver snapshot)
...
- En este host, el fallback local de Playwright puede requerir path de Node fijo (`>=18.19`) cuando la resolucion de shell no coincide.
